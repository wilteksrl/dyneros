...
const STATUS_LABELS: Record<string, string> = {
  in_development: "In Sviluppo", delivered: "Consegnato", planned: "Pianificato",
};
  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-xl font-semibold">AI & Automazioni</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Progetti AI e sistemi di automazione nel tuo workspace</p>
        </div>
        ...
        <div className="rounded-xl border p-5" style={{ background: CARD_BG, borderColor: "oklch(68% 0.19 72 / 0.15)" }}>
          <h2 className="text-sm font-semibold mb-2">Vuoi un nuovo agente AI?</h2>
          <p className="text-sm text-muted-foreground mb-4">
            Dyneros sviluppa agenti AI personalizzati per automazione compliance, analisi blockchain, customer support e molto altro.
          </p>
          <button className="flex items-center gap-2 px-4 h-9 rounded-lg text-sm font-medium transition-colors"
            style={{ background: GOLD, color: "#000" }}>
            <ExternalLink className="h-3.5 w-3.5" />
            Richiedi Progetto AI
          </button>
        </div>
      </div>
    </DashboardLayout>
