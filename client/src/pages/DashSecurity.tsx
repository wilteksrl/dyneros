        ...
        <div className="rounded-xl border p-5" style={{ background: CARD_BG, borderColor: "oklch(68% 0.19 72 / 0.2)" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {[
              ...
              { label: "2FA", value: data.twoFa ? "Attivo" : "Non attivo", icon: <Smartphone className="h-3.5 w-3.5" />, ok: data.twoFa },
              { label: "Sessioni Attive", value: `${data.activeSessions.length}`, icon: <Monitor className="h-3.5 w-3.5" />, ok: true },
            ].map(item => (
          ...
          <div className="space-y-2">
            {data.activeSessions.map((session, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-lg"
                ...
                {!session.current && (
                  <button onClick={() => toast.success("Sessione terminata")}
