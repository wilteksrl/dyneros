...
const STATUS_LABELS: Record<string, string> = {
  paid: "Pagata", unpaid: "Da Pagare", overdue: "Scaduta",
};
    ...
    <DashboardLayout>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-semibold">Fatture & Pagamenti</h1>
            <p className="text-sm text-muted-foreground mt-0.5">{invoices?.length ?? 0} fatture totali</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="rounded-xl border p-4" style={{ background: CARD_BG, borderColor: "oklch(68% 0.19 72 / 0.25)" }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Da Pagare</p>
            </div>
            <p className="text-2xl font-semibold" style={{ color: GOLD }}>
              €{totalDue.toLocaleString("it-IT")}
            </p>
          </div>
          <div className="rounded-xl border p-4" style={{ background: CARD_BG, borderColor: BORDER }}>
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle2 className="h-4 w-4 text-green-400" />
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Pagate (YTD)</p>
            </div>
            <p className="text-2xl font-semibold text-green-400">€{totalPaid.toLocaleString("it-IT")}</p>
          </div>
          <div className="rounded-xl border p-4" style={{ background: CARD_BG, borderColor: overdueCount > 0 ? "oklch(55% 0.22 25 / 0.3)" : BORDER }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-wide">Scadute</p>
            </div>
          ...
          <div className="space-y-3">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">Nessuna fattura in questa categoria</p>
              </div>
            )}
          </div>
        )}
