  ...
  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6">
        <div>
          <h1 className="text-xl font-semibold">Wallet & Assets</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Gestione self-custody — le chiavi private restano sempre in tuo possesso</p>
        </div>
        ...
        <div className="rounded-xl border p-5" style={{ background: CARD_BG, borderColor: "oklch(68% 0.19 72 / 0.2)" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold">Indirizzi Collegati</h2>
          </div>
        ...
        <div className="rounded-xl border p-5" style={{ background: CARD_BG, borderColor: BORDER }}>
          <h2 className="text-sm font-semibold mb-4">Portfolio Token</h2>
            ...
            {data.portfolio.map(token => (
              <div key={token.symbol} className="flex items-center gap-4 p-3 rounded-lg"
                ...
                <p className="text-sm font-semibold" style={{ color: GOLD }}>
                  {parseFloat(token.balance).toLocaleString("it-IT", { maximumFractionDigits: 2 })}
                </p>
              </div>
            ))}
          </div>
