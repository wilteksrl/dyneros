  ...
  return (
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <SectionCard title={t("label.notifications")} icon={<Bell className="h-4 w-4" style={{ color: GOLD }} />}>
          <ToggleRow label={t("settings.email_notif")} description={t("settings.email_notif_desc")} value={notifEmail} onChange={setNotifEmail} />
          <ToggleRow label={t("settings.ticket_notif")} description={t("settings.ticket_notif_desc")} value={notifTickets} onChange={setNotifTickets} />
          <ToggleRow label={t("settings.invoice_notif")} description={t("settings.invoice_notif_desc")} value={notifInvoices} onChange={setNotifInvoices} />
          <ToggleRow label={t("settings.milestone_notif")} description={t("settings.milestone_notif_desc")} value={notifMilestones} onChange={setNotifMilestones} />
        </SectionCard>

        <SectionCard title={t("settings.language_region")} icon={<Globe className="h-4 w-4" style={{ color: GOLD }} />}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted-foreground uppercase tracking-wide block mb-1.5">{t("label.language")}</label>
              <select value={lang} onChange={e => setLang(e.target.value as "it" | "en")}
                className="w-full h-9 px-3 rounded-lg text-sm bg-[oklch(13%_0.006_264)] border focus:outline-none"
                style={{ borderColor: BORDER }}>
                <option value="it">{t("label.italian")}</option>
                <option value="en">{t("label.english")}</option>
              </select>
            </div>
            <div>
              <label className="text-xs text-muted-foreground uppercase tracking-wide block mb-1.5">{t("settings.timezone")}</label>
        ...
        <SectionCard title={t("settings.appearance")} icon={<Moon className="h-4 w-4" style={{ color: GOLD }} />}>
          <div className="grid grid-cols-2 gap-3">
            {[
              { key: "dark" as const, label: t("label.dark"), desc: t("settings.dark_desc") },
              { key: "light" as const, label: t("label.light"), desc: t("settings.coming_soon") },
            ].map(item => (
              <div key={item.key}
                onClick={() => item.key === "dark" && setTheme(item.key)}
                className="p-3 rounded-lg border cursor-pointer transition-all"
                style={{
                  borderColor: theme === item.key ? "oklch(68% 0.19 72 / 0.4)" : BORDER,
                  background: theme === item.key ? "oklch(68% 0.19 72 / 0.05)" : "oklch(13% 0.006 264)",
                  opacity: item.key === "light" ? 0.5 : 1,
                }}>
